def generic_get_json_xml(request, model_name, app_name, relations={}, extras=('id',), indent=4, excludes=(), return_type='json', instance_id=None):
    """
    Retorna o json/xml do APP_NAME.MODEL_NAME informados.
    http://xxx/<app_name>/<model_name>/list.json
    http://xxx/<app_name>/<model_name>/list.xml

    EXEMPLOS:
    http://xxx/agenda/presenca/list.json?aluno__turma__id=1
    http://xxx/agenda/aluno/list.json?turma=1
    http://xxx/agenda/classificacao/list.json?tipo=10
    http://xxx/agenda/notificacao/list.json?dia_hora__gte=2012-01-01&dia_hora__lte=2012-01-01
    ...
    Onde o model_name é buscado diretamente da url, automagicamente todos os modelos estão extensíveis via json/xml
    (r'^(?P<model_name>\w+)/list[/]{0,1}.json$', 'util.generic_views.generic_get_json_xml' ),
    *pode ser colocado no URLS.PY RAIZ, desta forma informando a APP_NAME via url*
    (r'^(?P<app_name>\w+)/(?P<model_name>\w+)/list[/]{0,1}.json$', 'util.generic_views.generic_get_json_xml' ),

    PERSONALIZANDO O JSON:
    Para personalizar, passe parametros adicionais no urls.py.
    EX:
    (r'^atividade/list[/]{0,1}.(xml|json)$', 'util.generic_views.generic_get_json_xml', {
